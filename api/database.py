...
class Feature(Base):
    ...
    # Context weight: 1-5 scale estimating implementation complexity
    # 1=trivial, 2=small, 3=typical, 4=substantial, 5=complex
    context_weight = Column(Integer, nullable=False, default=3)
    # Failure tracking
    fail_count = Column(Integer, nullable=False, default=0)
    fail_reason = Column(Text, nullable=True)
    # Parallel agent identity — which agent claimed this feature
    claimed_by = Column(String(100), nullable=True, default=None)
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "context_weight": self.context_weight if self.context_weight is not None else 3,
            "fail_count": self.fail_count if self.fail_count is not None else 0,
...
def get_database_path(project_dir: Path) -> Path:
    ...
    return project_dir / ".autoforge" / "features.db"
...
def _migrate_add_forge_columns(engine) -> None:
    """Add context_weight, fail_count, fail_reason columns to existing databases."""
    with engine.connect() as conn:
        result = conn.execute(text("PRAGMA table_info(features)"))
        columns = [row[1] for row in result.fetchall()]

        if "context_weight" not in columns:
            conn.execute(text("ALTER TABLE features ADD COLUMN context_weight INTEGER DEFAULT 3"))
        if "fail_count" not in columns:
            conn.execute(text("ALTER TABLE features ADD COLUMN fail_count INTEGER DEFAULT 0"))
        if "fail_reason" not in columns:
            conn.execute(text("ALTER TABLE features ADD COLUMN fail_reason TEXT DEFAULT NULL"))
        conn.commit()


def _migrate_add_parallel_columns(engine) -> None:
    """Add claimed_by and claimed_at columns for parallel agent identity."""
    with engine.connect() as conn:
        result = conn.execute(text("PRAGMA table_info(features)"))
        columns = [row[1] for row in result.fetchall()]

        if "claimed_by" not in columns:
            conn.execute(text("ALTER TABLE features ADD COLUMN claimed_by VARCHAR(100) DEFAULT NULL"))
        if "claimed_at" not in columns:
            conn.execute(text("ALTER TABLE features ADD COLUMN claimed_at DATETIME DEFAULT NULL"))
        conn.commit()
...
def create_database(project_dir: Path) -> tuple:
    ...
    _migrate_add_forge_columns(engine)
    _migrate_add_parallel_columns(engine)
