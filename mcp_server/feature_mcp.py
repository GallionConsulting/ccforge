...
Provides 22 tools to manage features in the autonomous coding system.

Tools (Query):
- feature_get_stats: Get progress statistics (includes claimed feature details)
...
Tools (Lifecycle):
- feature_create: Create a single feature (with context_weight)
- feature_create_bulk: Create multiple features at once
- feature_update: Modify feature name/description/steps (resets fail state)
- feature_mark_in_progress: Mark a feature as in-progress (supports agent_id)
- feature_claim_and_get: Atomically claim and get feature details (supports agent_id)
- feature_mark_passing: Mark a feature as passing
- feature_mark_failing: Mark as failing with reason (increments fail_count)
- feature_skip: Skip a feature (move to end of queue)
- feature_clear_in_progress: Clear in-progress status for one feature
- feature_clear_all_in_progress: Reset orphaned in-progress features (supports agent_id filter)
- feature_clear_stale: Clear claims older than timeout (orphan detection in parallel mode)
...
class FeatureCreateItem(BaseModel):
    ...
    context_weight: int = Field(default=3, ge=1, le=5, description="Complexity: 1=trivial, 2=small, 3=typical, 4=substantial, 5=complex")
...
def feature_get_stats() -> str:
    ...
    try:
        ...
        # Get details of in-progress (claimed) features for orchestrator monitoring
        claimed = []
        if in_progress > 0:
            claimed_features = session.query(Feature).filter(
                Feature.in_progress == True
            ).all()
            for f in claimed_features:
                claimed.append({
                    "id": f.id,
                    "name": f.name,
                    "claimed_by": f.claimed_by,
                    "claimed_at": f.claimed_at.isoformat() if f.claimed_at else None,
                })
...
def feature_mark_passing(
    ...
    try:
        ...
        result = session.execute(text("""
            UPDATE features
            SET passes = 1, in_progress = 0, claimed_by = NULL, claimed_at = NULL
...
def feature_mark_failing(
    ...
    reason: Annotated[str, Field(default="", description="Structured explanation of why the feature failed")] = ""
) -> str:
    """Mark a feature as failing after finding a regression.

    Updates the feature's passes field to false, clears the in_progress flag,
    increments fail_count, and stores the failure reason.
    ...
    Args:
        ...
        reason: Explanation of why the feature failed (stored for /forge-fix)
    ...
    try:
        ...
        # Atomic update for parallel safety — increment fail_count and store reason
        params = {"id": feature_id, "reason": reason or None}
        session.execute(text("""
            UPDATE features
            SET passes = 0, in_progress = 0, claimed_by = NULL, claimed_at = NULL,
        ...
        return json.dumps({
            "message": f"Feature #{feature_id} marked as failing (attempt {feature.fail_count})",
...
def feature_mark_in_progress(
    ...
    agent_id: Annotated[str, Field(default="", description="Agent identifier for parallel mode (empty = anonymous)")] = ""
) -> str:
    ...
    Args:
        ...
        agent_id: Agent identifier for parallel mode (empty = anonymous)
    ...
    try:
        ...
        result = session.execute(text("""
            UPDATE features
            SET in_progress = 1, claimed_by = :agent_id, claimed_at = :now
            WHERE id = :id AND passes = 0 AND in_progress = 0
        """), {"id": feature_id, "agent_id": agent_id or None, "now": datetime.now(timezone.utc).isoformat()})
        session.commit()
...
def feature_clear_in_progress(
    ...
    try:
        ...
        session.execute(text("""
            UPDATE features
            SET in_progress = 0, claimed_by = NULL, claimed_at = NULL
            WHERE id = :id
        """), {"id": feature_id})
        session.commit()
        ...
        with atomic_transaction(_session_maker) as session:
            ...
            for i, feature_data in enumerate(features):
                db_feature = Feature(
                    ...
                    passes=False,
                    in_progress=False,
                    context_weight=feature_data.get("context_weight", 3),
                )
...
def feature_create(
    ...
    context_weight: Annotated[int, Field(default=3, ge=1, le=5, description="Complexity: 1=trivial, 2=small, 3=typical, 4=substantial, 5=complex")] = 3
) -> str:
    ...
    Args:
        ...
        context_weight: Complexity estimate 1-5 (default 3)
...
@mcp.tool()
def feature_get_progress_bar() -> str:
    """Get an ASCII progress bar showing overall feature completion.

    Returns a formatted progress bar with counts by status.
    Useful for quick status checks in /forge-status.

    Returns:
        Formatted ASCII progress bar with status breakdown
    """
    from sqlalchemy import case, func
    ...
    try:
        result = session.query(
            ...
            func.sum(case((Feature.passes == True, 1), else_=0)).label('passing'),
            func.sum(case((Feature.in_progress == True, 1), else_=0)).label('in_progress'),
            func.sum(case((Feature.fail_count > 0, 1), else_=0)).label('failing'),
        ).first()

        total = result.total or 0
        passing = int(result.passing or 0)
        in_progress = int(result.in_progress or 0)
        failing = int(result.failing or 0)
        pending = total - passing - in_progress - failing

        # Build ASCII progress bar (40 chars wide)
        bar_width = 40
        if total > 0:
            filled = round((passing / total) * bar_width)
        else:
            filled = 0
        empty = bar_width - filled
        bar = f"[{'=' * filled}{'>' if filled < bar_width else ''}{'.' * max(0, empty - 1)}]"
        if filled == bar_width:
            bar = f"[{'=' * bar_width}]"

        lines = [
            f"{bar} {passing}/{total} features passing",
            f"Pending: {pending} | In Progress: {in_progress} | Passing: {passing} | Failing: {failing}",
        ]
        return "\n".join(lines)
    finally:
        session.close()
...
@mcp.tool()
def feature_get_dependency_tree() -> str:
    """Get an ASCII dependency tree showing feature relationships and statuses.

    Renders a tree view of all features organized by their dependency
    relationships. Root features (no dependencies) are shown at the top level,
    with dependent features indented beneath their dependencies.

    Returns:
        Formatted ASCII dependency tree
    """
    session = get_session()
    try:
        ...
        for f in all_features:
            for dep_id in (f.dependencies or []):
                if dep_id in children:
                    children[dep_id].append(f.id)
                    has_parent.add(f.id)

        # Status label
        def status_label(f: Feature) -> str:
            if f.passes:
                return "PASS"
            if f.in_progress:
                return "PROG"
            deps = f.dependencies or []
            if any(d not in passing_ids for d in deps):
                return "BLKD"
            if f.fail_count and f.fail_count > 0:
                return "FAIL"
            return "PEND"
        ...
        def render(fid: int, prefix: str, is_last: bool, is_root: bool) -> None:
            if fid in rendered or fid not in feature_map:
                return
            rendered.add(fid)
            f = feature_map[fid]
            connector = "" if is_root else ("+-- " if is_last else "|-- ")
            lines.append(f"{prefix}{connector}{f.id}. [{status_label(f)}] {f.name}")

            child_ids = sorted(children.get(fid, []))
            child_prefix = prefix if is_root else (prefix + ("    " if is_last else "|   "))
            for i, child_id in enumerate(child_ids):
                render(child_id, child_prefix, i == len(child_ids) - 1, False)

        # Render root features (no dependencies)
        roots = [f.id for f in all_features if f.id not in has_parent]
        for fid in roots:
            render(fid, "", True, True)

        # Render any orphaned features not yet rendered
        for f in all_features:
            if f.id not in rendered:
                render(f.id, "", True, True)

        return "\n".join(lines) if lines else "No features found."
    finally:
        session.close()
...
@mcp.tool()
def feature_update(
    feature_id: Annotated[int, Field(ge=1, description="The ID of the feature to update")],
    name: Annotated[str, Field(default="", description="New feature name (empty = no change)")] = "",
    description: Annotated[str, Field(default="", description="New feature description (empty = no change)")] = "",
    steps: Annotated[str, Field(default="", description="New verification steps as JSON array string (empty = no change)")] = ""
) -> str:
    """Update an existing feature's name, description, and/or verification steps.

    Modifying a feature resets fail_count to 0 and clears fail_reason,
    since editing implies a fresh attempt. Preserves the feature's ID,
    dependencies, and position in the graph.

    Used by /forge-fix to let users revise broken feature specs.

    Args:
        feature_id: The ID of the feature to update
        name: New name (leave empty to keep current)
        description: New description (leave empty to keep current)
        steps: New steps as JSON array string (leave empty to keep current)
    ...
    try:
        ...
        if steps:
            try:
                ...
                return json.dumps({"error": "Steps must be valid JSON array string"})

        if not changed:
            return json.dumps({"error": "No fields to update — provide at least one of: name, description, steps"})

        # Reset failure tracking on edit (fresh attempt)
        feature.fail_count = 0
        ...
        return json.dumps({
            "success": True,
            "message": f"Feature #{feature_id} updated (fail state reset)",
            "feature": feature.to_dict()
        })
    except Exception as e:
        session.rollback()
        return json.dumps({"error": f"Failed to update feature: {str(e)}"})
    finally:
        session.close()


@mcp.tool()
def feature_clear_all_in_progress(
    agent_id: Annotated[str, Field(default="", description="Only clear claims by this agent (empty = clear all)")] = ""
) -> str:
    """Reset in-progress features (that haven't passed) back to pending.

    Cleans up orphaned in-progress features from crashed agent sessions.
    Only affects features where passes=0 and in_progress=1.

    When agent_id is provided, only clears that agent's claims (safe for parallel mode).
    When empty, clears all orphaned claims (backwards-compatible single-agent mode).

    Called by /forge-build and /forge-parallel as their first action to ensure clean state.

    Args:
        agent_id: Only clear claims by this agent (empty = clear all)

    Returns:
        JSON with count of cleared features and their IDs/names
    """
    session = get_session()
    try:
        # Find orphaned in-progress features (optionally filtered by agent)
        query = session.query(Feature).filter(
            Feature.in_progress == True,
            Feature.passes == False
        )
        if agent_id:
            query = query.filter(Feature.claimed_by == agent_id)

        orphaned = query.all()

        if not orphaned:
            return json.dumps({"cleared": 0, "features": []})

        cleared = []
        for f in orphaned:
            cleared.append({"id": f.id, "name": f.name})
        ...
        if agent_id:
            session.execute(text("""
                UPDATE features
                SET in_progress = 0, claimed_by = NULL, claimed_at = NULL
                WHERE in_progress = 1 AND passes = 0 AND claimed_by = :agent_id
            """), {"agent_id": agent_id})
        else:
            session.execute(text("""
                UPDATE features
                SET in_progress = 0, claimed_by = NULL, claimed_at = NULL
                WHERE in_progress = 1 AND passes = 0
            """))
        session.commit()
        ...
        session.rollback()
        return json.dumps({"error": f"Failed to clear in-progress features: {str(e)}"})
    finally:
        session.close()


@mcp.tool()
def feature_clear_stale(
    timeout_minutes: Annotated[int, Field(default=30, ge=5, le=120, description="Clear claims older than this many minutes")] = 30
) -> str:
    """Clear in-progress features whose claims are older than timeout.

    Detects orphaned claims from crashed agents in parallel mode.
    Only clears features with a claimed_at timestamp older than the threshold.
    Features without claimed_at are NOT cleared (backwards compat).

    Args:
        timeout_minutes: Minutes after which a claim is considered stale (5-120, default 30)
    ...
    try:
        ...
        # Find stale features for reporting
        stale = session.query(Feature).filter(
            Feature.in_progress == True,
            Feature.passes == False,
            Feature.claimed_at.isnot(None),
            Feature.claimed_at < cutoff
        ).all()
        ...
        cleared = []
        for f in stale:
            cleared.append({"id": f.id, "name": f.name, "claimed_by": f.claimed_by, "claimed_at": f.claimed_at.isoformat() if f.claimed_at else None})
        ...
        session.rollback()
        return json.dumps({"error": f"Failed to clear stale features: {str(e)}"})
    finally:
        session.close()
