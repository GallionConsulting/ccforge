...
[
  ...
  { "name": "User can filter todos", "context_weight": 3, "depends_on_indices": [0, 1, 2, 3, 4, 12] },
  { "name": "User can search todos", "context_weight": 3, "depends_on_indices": [0, 1, 2, 3, 4, 12] }
]
```
...
Your features **MUST** include tests from ALL 20 categories. The table below shows reference distribution by complexity tier — scale proportionally to your actual `feature_count`.
...
Your features must include tests that **actively verify real data** and **detect mock data patterns**.
