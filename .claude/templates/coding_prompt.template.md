...
Understanding the app spec is critical - it contains the full requirements
...
Call `feature_get_ready` to get the next ready feature (dependencies satisfied, not in progress). Then use `feature_get_by_id` to get the full details. Mark it as in-progress:
...
Document the SPECIFIC external blocker in `.autoforge/progress_notes.md`. "Functionality not built" is NEVER a valid reason.
...
Update `.autoforge/progress_notes.md` with:
...
2. Update .autoforge/progress_notes.md
...
# 2. Get next ready feature (dependencies met, not in-progress)
feature_get_ready

# 3. Get feature details by ID
feature_get_by_id with feature_id={id}

# 4. Mark a feature as in-progress
...
- Use `feature_get_ready` to find the next feature to work on
- Do NOT query features by category or try to fetch all features at once
...
**Remember:** Zero console errors. All data from real database. Leave codebase clean before ending session.
