...
>
> 1. **Yes, generate files** - Create app_spec.txt, project setup, and all config files
...
Once the user approves, generate ALL of the following files:
...
- Copy `.claude/templates/coding_prompt.template.md` to `$ARGUMENTS/.autoforge/prompts/coding_prompt.md`
- Copy `.claude/templates/testing_prompt.template.md` to `$ARGUMENTS/.autoforge/prompts/testing_prompt.md`
...
Claude Code uses `.mcp.json` in the project root for per-project MCP server configuration. Generate this file so the feature MCP server is available to all forge commands:
...
```json
{
  "mcpServers": {
    "feature-mcp": {
      "type": "stdio",
      "command": "python",
      "args": ["mcp_server/feature_mcp.py"],
      "env": { "PROJECT_DIR": "." }
    }
  }
}
...
- Copy `mcp_server/` directory (feature_mcp.py, requirements.txt, __init__.py) to `$ARGUMENTS/mcp_server/`
- Copy `api/` directory (database.py, dependency_resolver.py, __init__.py) to `$ARGUMENTS/api/`
...
Generate from the template at `.claude/templates/project-claude.template.md`, filling in:

- `[PROJECT_NAME]` — the project name from Phase 1
- `[PROJECT_DESCRIPTION]` — the overview from Phase 1
...
```bash
mkdir -p $ARGUMENTS/.claude/commands
for cmd in forge-init.md forge-build.md forge-parallel.md forge-status.md forge-test.md forge-fix.md forge-expand.md forge-regression.md checkpoint.md check-code.md review-pr.md; do
  cp .claude/commands/$cmd $ARGUMENTS/.claude/commands/
done
```
...
for agent in coder.md code-review.md deep-dive.md; do
  cp .claude/agents/$agent $ARGUMENTS/.claude/agents/
done
```
...
```bash
mkdir -p $ARGUMENTS/.autoforge/prompts
```

Also create `.autoforge/.gitignore` with:
...
> "Your project has been set up at `$ARGUMENTS/`!
>
> **Files created:**
...
> - `$ARGUMENTS/CLAUDE.md` — Project instructions for Claude Code
...
> ---
>
> ### **⚠️ IMPORTANT: RESTART Claude Code before continuing!**
>
> **The forge commands were just copied into your project directory. Claude Code must be restarted to detect them.**
>
> **Close Claude Code completely and reopen it in your project directory before running `/forge-init`.**
>
> ---
>
> **Next steps:**
...
> 2. `cd $ARGUMENTS`
> 3. Run `/forge-init` to create features in the database from your spec
> 4. Then `/forge-build` or `/forge-parallel` to start building features
>
> **How it works:**
>
> - `/forge-init` reads your spec and creates trackable features in a SQLite database
> - `/forge-build` picks up the next ready feature and implements it sequentially
> - `/forge-parallel` builds multiple features at once with concurrent workers
> - `/forge-status` shows current progress at any time
> - `/forge-test` runs interactive QA across completed features
> - `/forge-expand` adds new features to an existing project
>
> **Tip:** Always run `/clear` between different forge commands to ensure a fresh context window.
>
> Building all [X] features will take multiple `/forge-build` sessions (or fewer `/forge-parallel` runs). Each `/forge-build` session implements one or more features."
