...
This project uses the Forge autonomous development system. Key commands:

- `/forge-build` — Main build loop: implements the next ready feature(s)
- `/forge-status` — Check current progress and feature status
- `/forge-test` — Run regression tests across completed features
- `/forge-fix` — Interactive failure resolution and feature editing

Feature tracking is managed via an MCP server (`mcp_server/feature_mcp.py`) with tools like `feature_get_stats`, `feature_get_by_id`, `feature_mark_passing`, etc. Features are stored in `.autoforge/features.db` (SQLite).

Always check:
- `app_spec.txt` for the project specification
- `CLAUDE.md` for project-specific instructions and conventions
- `.autoforge/features.db` for feature status (via MCP tools)
- `init.sh` for the development environment setup script
- `.claude/skills/playwright-cli/` for browser automation testing
